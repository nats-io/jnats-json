...
public final class IndexedBuilderTests {
    ...
    public void testMapBuilderNullPaths() {
        ...
        assertEquals("{}", IndexedMapBuilder.instance().putEntries(null).toJson());

        // explicit NULL sentinels are treated as null
        assertEquals("{}", IndexedMapBuilder.instance()
            .put("a", JsonValue.NULL).put("b", IndexedJsonValue.NULL).toJson());
        assertEquals("{\"a\":null,\"b\":null}", IndexedMapBuilder.instance(true)
            .put("a", JsonValue.NULL).put("b", IndexedJsonValue.NULL).toJson());
        ...
        assertEquals("{\"a\":1}", IndexedMapBuilder.instance().putEntries(src).toJson());
        assertEquals("{\"a\":1,\"b\":null}", IndexedMapBuilder.instance(true).putEntries(src).toJson());
    }

    @Test
    public void testArrayBuilderNullPaths() {
        // null collection argument is a no-op
        assertEquals("[]", IndexedArrayBuilder.instance().addItems(null).toJson());

        // explicit NULL sentinels are treated as null
        assertEquals("[]", IndexedArrayBuilder.instance()
            .add(JsonValue.NULL).add(IndexedJsonValue.NULL).toJson());
        assertEquals("[null,null]", IndexedArrayBuilder.instance(true)
            .add(JsonValue.NULL).add(IndexedJsonValue.NULL).toJson());

        // addItems drops null items when addNulls is false, keeps them when true
        assertEquals("[\"a\",\"b\"]",
            IndexedArrayBuilder.instance().addItems(Arrays.asList("a", null, "b")).toJson());
        assertEquals("[\"a\",null,\"b\"]",
            IndexedArrayBuilder.instance(true).addItems(Arrays.asList("a", null, "b")).toJson());
    }
    ...
    public void testConstructorsAndFactories() {
        ...
        assertFalse(new IndexedMapBuilder().allowPutNulls());
        assertTrue(new IndexedMapBuilder(true).allowPutNulls());
        assertFalse(IndexedMapBuilder.instance().allowPutNulls());
        assertTrue(IndexedMapBuilder.instance(true).allowPutNulls());
        assertEquals("{\"k\":1}", new IndexedMapBuilder().put("k", 1).toJson());
        assertEquals("{\"k\":null}", new IndexedMapBuilder(true).put("k", null).toJson());

        // array builder: no-arg ctor, boolean ctor, instance(), instance(boolean)
        assertFalse(new IndexedArrayBuilder().allowAddNulls());
        assertTrue(new IndexedArrayBuilder(true).allowAddNulls());
        assertFalse(IndexedArrayBuilder.instance().allowAddNulls());
        assertTrue(IndexedArrayBuilder.instance(true).allowAddNulls());
        assertEquals("[1]", new IndexedArrayBuilder().add(1).toJson());
        assertEquals("[null]", new IndexedArrayBuilder(true).add(null).toJson());
    }

    @Test
    public void testValueLeafConstructorsAndValidation() {
        // valid leaf constructors
        assertEquals("hello", new IndexedJsonValue("hello").getString());
        assertEquals(Integer.valueOf(7), new IndexedJsonValue(JsonValueType.INTEGER, 7).getInteger());
        assertEquals(Long.valueOf(7L), new IndexedJsonValue(JsonValueType.LONG, 7L).getLong());
        assertEquals(Double.valueOf(1.5d), new IndexedJsonValue(JsonValueType.DOUBLE, 1.5d).getDouble());

        // validation: STRING requires a non-null string
        assertThrows(IllegalArgumentException.class, () -> new IndexedJsonValue((String) null));
        // validation: a numeric type requires a non-null number
        assertThrows(IllegalArgumentException.class, () -> new IndexedJsonValue(JsonValueType.INTEGER, (Number) null));
        // validation: the cache constructor only supports STRING and numeric types
        assertThrows(IllegalArgumentException.class, () -> new IndexedJsonValue(JsonValueType.BOOL, 1));
    }
    ...
    public void testToJsonValue() {
        ...
        IndexedMapBuilder b = IndexedMapBuilder.instance()
            .put("s", "x")
            .put("i", 1)
            .put("l", 3000000000L)
            .put("d", 1.5d)
        ...
        // build() returns the indexed value; toJsonValue() returns a materialized JsonValue view
        assertEquals(IndexedJsonValue.class, b.build().getClass());
        ...
        JsonValue arrFromBuilder = IndexedArrayBuilder.instance().add("y").add(1).add(true).toJsonValue();
